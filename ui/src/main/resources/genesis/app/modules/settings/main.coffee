  ...
  class AppSettings.Views.Main extends Backbone.View
    ...
    render: ->
      $.when(@model.fetch(), genesis.fetchTemplate(@template)).done (linksAggregator, tmpl) =>
        ...
        @$el.html tmpl(
          users: typeToLink[backend.LinkTypes.User]
          groups: typeToLink[backend.LinkTypes.UserGroup]
          roles: typeToLink[backend.LinkTypes.Role]
          plugins: typeToLink[backend.LinkTypes.Plugin]
          databags: typeToLink[backend.LinkTypes.DataBag]
          agents: typeToLink[backend.LinkTypes.RemoteAgent]
        )
        @showSettings()
