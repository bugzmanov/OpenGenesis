...
(genesis, Backbone, status, property, validation, backend, $) ->
  ...
  class Databags.Model extends Backbone.Model
    ...
    parse: (json) ->
      if(json.items)
        @_editLink = _(json.links).find((link) -> link.rel == "self" && _(link.methods).contains("put"))
        @_deleteLink = _(json.links).find((link) -> link.rel == "self" && _(link.methods).contains("delete"))
        json.items
      else
        json
    ...
    clone: ->
      c = super.clone()
      c._editLink = @_editLink
      c._deleteLink = @_deleteLink
      c
  ...
  class Databags.Collection extends Backbone.Collection
    ...
    parse: (json) ->
      if(json.items)
        @_createLink = _(json.links).find((link) -> link.type == backend.LinkTypes.DataBag and _(link.methods).contains("post"))
        json.items
      else
        json

    canCreate: ->
      not _.isUndefined(@_createLink)
