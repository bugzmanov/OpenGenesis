...
//  def apply(webPath: WebPath, rel: LinkTarget, modelClazz: Class[_], methods: RequestMethod*) =
//    Link(webPath.toString, rel.toRel, modelClazz, (methods.toList).map(_.toString.toLowerCase).toArray)
//
